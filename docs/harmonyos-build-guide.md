...
git clone https://gitee.com/openharmony-sig/flutter_flutter.git -b 3.22.0-ohos
cd flutter_flutter
...
# 下载 ohos 版 flutter
git clone -b 3.22.0-ohos https://gitee.com/openharmony-sig/flutter_flutter.git
```
...
| 依赖 | 鸿蒙支持 | 说明 |
|------|---------|------|
| flutter_riverpod | ✅ 支持 | 纯 Dart 库 |
| dio | ✅ 支持 | 纯 Dart 网络库 |
| cached_network_image | ⚠️ 需适配 | 需使用 ohos 兼容版本或替换 |
| google_fonts | ⚠️ 需替换 | 鸿蒙使用系统字体，建议移除 |
| shimmer | ✅ 支持 | 纯 Dart 实现 |
| intl | ✅ 支持 | 纯 Dart 库 |
| cupertino_icons | ✅ 支持 | 资源包 |

### 鸿蒙推荐替换方案

| Android/iOS | 鸿蒙推荐 |
|------------|---------|
| cached_network_image | `cached_network_image_ohos` 或自行实现 |
| google_fonts | 使用系统 `HarmonyOS Sans` 字体 |
| flutter_secure_storage | `ohos_keystore` 插件 |
